...
class OpenInterestCollector(BaseCollector):
    ...
    def collect(self, exchange: Any, symbols: List[str], store: Any) -> int:
        ...
        for compact_sym in symbols:
            ...
            # Bitget's OI endpoint returns amount (base) but not USD value or
            # mark price. Derive oi_usd from the latest funding rate row in the
            # store which already has mark_px — no extra API call needed.
            if oi_usd is None and oi_base is not None:
                try:
                    fr_rows = store.get_latest("funding_rate", compact_sym, n=1)
                    if fr_rows:
                        stored_mark = fr_rows[0].get("mark_px")
                        if stored_mark:
                            mark_px = mark_px or float(stored_mark)
